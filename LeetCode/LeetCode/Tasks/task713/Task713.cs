...
namespace LeetCode.Tasks.task713
{
    /*
     713. Произведение подмассивов меньше K
    Учитывая массив целых чисел nums и целое число k, верните количество смежных подмассивов, в которых произведение всех элементов подмассива строго меньше k.
    Ограничения:
     ...
     */
    public class Task713 : InfoBasicTask
    {
            ...
            if (isValid(nums, k))
            {
                ...
                switch (typeSolution)
                {
                    case TypeSolution.BruteForce:
                        ...
                        Console.WriteLine($"Решение с помощью метода BruteForce: количество подмассивов, произведение элементов которых строго меньше, чем {k} = {res}");
                        break;
                    case TypeSolution.OptimalAlgorithm:
                        res = optimalAlgorithm(nums, k);
                        Console.WriteLine($"Решение с помощью оптимального метода: количество подмассивов, произведение элементов которых строго меньше, чем {k} = {res}");
                        break;
        ...
        private bool isValid(int[] nums, int k)
        {
            ...
            int highLimitLengthNums = 3*(int)Math.Pow(10,4);
            int lowLimitValueInNums = 1;
            int highLimitValueInNums = 1000;
            int lowLimitK = 0;
            int highLimitK = (int)Math.Pow(10, 6);
        ...
        private int optimalAlgorithm(int[] nums, int k) // скопировано с leetcode
        {
            var result = 0;
            var leftIndex = 0;
            var rightIndex = 0;
            ...
            while (rightIndex < nums.Length)
            {
                ...
                if (product >= k)
                {
                    while (product >= k && leftIndex <= rightIndex)
                    {
        ...
        private TypeSolution askUserTypeSolution()
        {
            while (true)
            {
                Console.WriteLine("Выберите тип решения :\n" +
                    "1 - BruteForce\n" +
                    "2 - Оптимальный алгоритм с применением скользящего окна\n" +
                ...
                try
                {
                    ...
                    switch (choiceUser)
                    {
                        ...
                        case 2:
                            return TypeSolution.OptimalAlgorithm;
                        case 3:
